...
#include "com_jing_libjpegcompress_CompressUtils.h"

#define LOG_TAG "chenyongjing"
...
//    LOGW("jpeg_msg_table[%d]:%s", myerr->pub.msg_code, myerr->pub.jpeg_message_table);
...
generateJPEG(BYTE *data, int w, int h, int quality, const char *outfilename, jboolean optimize) {
    //jpeg的结构体，保存宽、高、位深、图片格式等信息，相当于Java的类
    struct jpeg_compress_struct jcs;

    //当读完整个文件是就会回调my_error_exit这个方法退出，setjmp是一个系统级函数，是一个回调
...
JNIEXPORT jstring JNICALL Java_com_jing_libjpegcompress_CompressUtils_compressBitmap
        (JNIEnv *env, jclass thiz, jobject bitmapcolor, jint w, jint h, jint quality,
